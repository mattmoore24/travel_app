 ...
 *
 * One refusal: an hour already gone on the CITY's clock is not a plan. There
 * used to be a second, at the pin's own expiry, and it is gone on purpose. A
 * take-down day BEFORE the plan is a legal, ordinary choice now (the founder
 * overruled the floor on 2026-09-10 and the server no longer refuses an hour
 * past the take-down), so keeping the ceiling here would have made the From
 * and To rails silently vanish for exactly the traveler who took him up on
 * it, which is a harder block than the confirmation he refused.
 *

...
select isnt(
  (select updated_at from public.profiles where user_id = '00000000-0000-0000-0000-0000000000d1'),
  null,
...
create function pg_temp.before() returns timestamptz language sql stable as
  $$ select pg_temp.stamp() $$;

select set_config('request.jwt.claims',
  json_build_object('sub', '00000000-0000-0000-0000-0000000000d1',
                    'role', 'authenticated')::text, true);

-- Captured before the call, compared after.
create temp table mark as select pg_temp.stamp() as was;
...
select is(
  pg_temp.stamp(), (select was from mark),

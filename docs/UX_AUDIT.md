...
| H   | A business's Map tab opens on the wrong city, thousands of miles from its own door _(closed 2026-09-05: the map reads the business's city by id, any city)_ | flow                     | S      | confirmed        | `src/features/pins/map-screen.tsx:687-688 (`activeCityId = cityId ?? launchCities[0]?.city_id`); screenshots 70-business-my-business.png and 71-busines` |
...
| M   | Four cities, no explanation, and no path for a business anywhere else _(closed 2026-09-05: one address box, any city, and a set-the-pin-yourself line)_     | empty-state              | S      | confirmed        | `screenshot 42-business-where-empty.png; src/app/business-signup.tsx:355-379 (chips from useLaunchCities, Continue blocked until one is picked)`         |

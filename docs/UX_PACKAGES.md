...
**Status: done (2026-09-01) in the tree, migration 20260902260000, on a founder call taken by Claude.** The "Waits on" question below was decided under the founder's blanket approval and is recorded as the D14 override in [`UX_PLAN.md`](UX_PLAN.md). `featured_traveler()` returns three with every earlier guard restated, `is_blocked_pair` added, `distinct on (user_id)` and a total order so the card call and the photo call cut the same people, and the photos keyed by `user_id` rather than list position.
...
**Status: done (2026-09-02), batched with the 0.2.0 build; the 2026-09-01 deferral is history.** Everything the deferral said was missing is in the tree: `expo-store-review ~57.0.2` in package.json, `version` 0.2.0 in app.json and package.json (the bump that moves `runtimeVersion` with it), the ask in `useAcceptedCelebration` a beat after the notice's X and never from "Go to chat", and `src/features/matching/__tests__/use-accepted-celebration.test.tsx`, where every gate was shown failing under the mutation that removes it. Five bad-moment gates rather than the "block or report" the Changes block below names: the account's standing, a hello refused in the last day, a block this session, a chat closed this session, and a photo of theirs refused (a profile photo in the last day, or a group photo watched being refused; a report is still the one the hook cannot see). The write-up is under "Shipped in 0.2.0: the App Store review prompt" in [`APP_STORE.md`](APP_STORE.md), with the TestFlight hand-run (the dialog cannot appear there by Apple's design, so the `review_prompt_requested` event is the evidence) and the App Store one. What it still waits on is the build itself: on build 16 the module is absent, the late require is contained, and the hook never asks.
...
**Status: done (2026-09-01), seven routes.** The table that used to sit here was verified row by row against the code before it was applied, and it was right about all six of its open rows, including the two that say "do nothing". What each route got:

- `archived-chats` — `headerTitle: 'Archived'` on the route, in-body title deleted (the earlier pass).
- `first-messages` — `headerTitle: 'Waiting on you'` on the route, the inbox section's own words (`(tabs)/chat.tsx`); the `<ThemedText type="title">` under the chevron is deleted, the footnote and the cards stay.
- `place/[id]` — the business name, set from INSIDE the loaded branch (`<Stack.Screen options={{ headerTitle: place.name }} />`), which is the one mechanism this repo already had for a title that arrives with a query (`profile/[userId]`), so nothing new was invented: no `headerTitle` render callback, no `useNavigation().setOptions`. The layout keeps `''` as the pre-resolve placeholder under the skeleton and the error state. The hero keeps its own copy of the name on purpose, because that copy carries `<PlaceSeal />` and the category/hours line and a native title cannot.
- `join-group/[token]` — `headerTitle: 'Group invite'` on the route; every branch headline kept (open, not open, ended, already in, not for a business).
- `i/[token]` — the same `'Group invite'`. It is the same screen re-exported, and `stack-header.test.ts` holds the two strings equal.
- `profile-me` — nothing. Its four page branches already set `'Your profile'` / `'Account'` / `'Your profile'` / `'Your profile'` themselves; the optional lift of the static ones into the layout was not taken, because one mechanism per screen is worth more than the first frame. (The keyless-dev `PlaceholderScreen` branch is the one page there with no title, and no phone renders it.)
- `profile/[userId]` — nothing. `''` in the layout, `headerTitle: name` from the screen once the query lands, exactly as the table said.

**The mechanism, from the code rather than from memory.** `<Stack.Screen options>` inside a component is `navigation.setOptions` in a layout effect (`expo-router/build/views/Screen.js`), so the row is empty for one frame on push, which is tolerable for a title and was not for `headerShown` (why `room/[id]`'s is in the layout). The title's colour is `headerTitleStyle.color ?? headerTintColor ?? colors.text` (expo-router's native-stack, `useHeaderConfigProps.js:136-138`), and `NavigationTheme.colors.text` is `Colors.dark.text`, so the three new titles paint in Nocturne's own text token and the chevron in `colors.primary`, the accent. No simulator ran this round, so that is read from the code, not from a picture; the same path has been painting `'Archived'`, `'Your profile'` and `'Quick replies'` since those landed, so nothing new is being trusted.

**Two levels deep, on the stack where it matters.** react-native-screens builds a back button from the CURRENT screen's config and the PREVIOUS item's title: `configureBackItem` (`RNSScreenStackHeaderConfig.mm:702-716`) writes `prevItem.backButtonTitle = prevItem.title` and `prevItem.backButtonDisplayMode = config.backButtonDisplayMode`. So on `(tabs)` → `first-messages` → `profile/[userId]` (the face on a card), the profile's config carries the root's global `headerBackButtonDisplayMode: 'minimal'`, its back button stays a bare chevron, and the only thing that changes is that the long-press back menu on that chevron now reads "Waiting on you" where it read a blank entry. Remove `minimal` and the same chevron prints "Waiting on you" beside itself, shortened to "Back" by UIKit when the person's name needs the width. That is why `stack-header.test.ts` pins `minimal` on the root `<Stack>`. On `(tabs)` → `place/[id]` → `room/[id]` the room's native header is `headerShown: false` (it draws `features/chat/thread-header` with its own "Back" control) and every other exit from the business page is a modal with no header, so the business name cannot reach a back button anywhere; after `join-group`, `guest-name` is a modal and the room is a `replace`, so nothing after "Group invite" has a native back button either.

**Maestro.** `e2e/` was grepped for every string this moved or added ("Waiting on you", "Group invite", "Archived", the seeded business name). No flow asserts any of them: the two `'Archived'` steps are on the Chat tab's row, and the business page is asserted through "See the whole page" and "Website". No step moves, and no spoken label changed.

**Tests.** `src/app/__tests__/first-messages.test.tsx` renders the page: the body no longer draws the title, the footnote and the cards remain, "Nothing waiting on you" is still a body state, and the layout carries the title. `place-title.test.tsx` renders the business page with a capturing `Stack.Screen`: nothing is set while pending or on a failed fetch, the name once loaded, one copy on the hero with "Verified business" beside it. `stack-header.test.ts` pins the root's `minimal`, the four static titles, the invite pair equal, the two query-set routes at `''` in the layout and setting nothing before their loaded anchor, and one title per page in `profile-me`. Every assertion was run against the mutation that removes what it names (the title drawn in the body again; the layout title back to `''`; the place screen setting no title, and setting one from the skeleton; `i/[token]` drifting to `'Invite'`; `minimal` removed; a `profile-me` branch losing its title; the hero name drawn twice; `profile/[userId]` setting `undefined`) and each failed on exactly that assertion.

**Scope, against the Changes block below.** The eight routes it lists are not the seven above: `chat/[id]`, `group/[id]` and `room/[id]` are `headerShown: false` and draw the one-storey `thread-header`, which is what the thread packages did, and `add-people/[chatId]` and `add-to-group/[userId]` are modals presented under the root's `headerShown: false`, which never had a bare back button. The seven `headerTitle: ''` routes were the whole of the pattern this package names, and all seven are accounted for.
...
**Status: done (2026-09-02), migration 20260903040000.** `admin_verification_queue` and `admin_business_verification_queue` exist as specified below: service-role views for the SQL editor, `reason` beside `reason_en`, `revoke all ... from anon, authenticated` on the line after each `create`, no RPC, no client. `supabase/tests/database/66_a_verdict_the_founder_can_read.test.sql` is the attack, and each refusal was shown to come back "lives" with its revoke deleted. Both views are named in DASHBOARD.md's admin query set and in LAUNCH_RUNBOOK.md's "Watch the numbers", so the founder is told to open them during an appeal rather than left to remember they exist.
...
_Retired 2026-09-05: the founder opened every city to businesses; the chips, the launch-state sentence and the /contact door went, replaced by one address field and a set-the-pin-yourself line. The `city_interest` table stays not built; the signal is `businesses.city_id` itself._
...
**Waits on.** ~~Do we own samewhere.app~~ — ANSWERED 2026-09-09: **no.** The founder owns samewhere.io only, and `hello@samewhere.io` is the one live mailbox. So universal links would have to be built on samewhere.io, not samewhere.app. Remaining question: do we spend an EAS build on universal links now? FOR: a custom-scheme link is dead for anyone who does not already have the app, which is every person a hostel is trying to reach, so the share feature only half works without it. AGAINST: it costs an EAS build, an apple-app-site-association file on a domain that may not exist yet, and an app.json change, and the QR at the counter — the case §2.6 actually names — works today without any of it.
...
| A `city_requests` / `city_interest` table for businesses outside the four launch cities       | /contact already exists and is already used from profile-me.tsx:154, so the demand signal can be captured today with zero schema. A table only earns its place once somebody is actually reading the answers, and nobody is. (If it is _Retired 2026-09-05: the founder opened every city to businesses; the chips and the /contact door on step 5 went, replaced by one address field and a set-the-pin-yourself line. The `city_interest` table stays not built; the signal is `businesses.city_id` itself._ |
